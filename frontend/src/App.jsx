...
function App() {
  ...
  const backgrounds = [
    ...
    { name: 'Peaceful Zen',    url: 'https://res.cloudinary.com/ninjagrvl/image/upload/v1771394783/ec9wd50vlrvcaeocf90y.png' },
    { name: 'Rainforest',      url: 'https://res.cloudinary.com/ninjagrvl/image/upload/v1771398228/njxhneme7ifznyt7zk6f.png' },
    { name: 'Tropical Jungle', url: 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?auto=format&fit=crop&w=1200&q=80' },
    { name: 'Desert Night',    url: 'https://images.unsplash.com/photo-1509316785289-025f5b846b35?auto=format&fit=crop&w=1200&q=80' },
  ];
  ...
  // ✅ FIX 1: folder is "frequency" not "frequencies"
  const sounds = {
    sound1: '/audio/2.mp3',
    sound2: '/audio/3.mp3',
    sound3: '/audio/4.mp3',
  ...
  const [soundStates, setSoundStates] = useState({ sound1: false, sound2: false, sound3: false, sound4: false, sound5: false, sound6: false });
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      if (lightPanelRef.current && !lightPanelRef.current.contains(e.target)) setShowLightPanel(false);
    };
  ...
  const soundCards = [
    ...
    { key: 'sound5', label: 'Tropical Rainforest', color: 'from-green-500 to-emerald-800', glow: 'shadow-green-500/40',  icon: '🌴', activeRing: 'ring-green-500'  },
    { key: 'sound6', label: 'Desert Night',        color: 'from-orange-400 to-amber-800',  glow: 'shadow-orange-400/40', icon: '🌵', activeRing: 'ring-orange-400' },
  ];
  ...
  return (
    <div className={`min-h-screen w-full relative overflow-x-hidden font-sans transition-all duration-700 ${
      darkMode ? 'bg-[#060a0f] text-slate-100' : 'bg-gradient-to-br from-emerald-50 via-white to-sky-100 text-slate-900'
    }`}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .particle { position: absolute; border-radius: 50%; background: rgba(148,163,184,0.4); animation: floatParticle linear infinite; }
        ...
        .title-glow { text-shadow: 0 0 30px rgba(34,211,238,0.5), 0 0 60px rgba(34,211,238,0.2), 0 0 100px rgba(34,211,238,0.1); }
        .sound-btn { position: relative; overflow: hidden; transition: all 0.3s cubic-bezier(0.34,1.56,0.64,1); }
        .sound-btn::before { content:''; position:absolute; inset:0; border-radius:inherit; background:inherit; filter:blur(15px); opacity:0; transition:opacity 0.3s; z-index:-1; transform:scale(1.2); }
        ...
        .breath-orb.exhale { transform: scale(1);   box-shadow: 0 0 20px rgba(16,185,129,0.3), 0 0 40px rgba(16,185,129,0.1); }
        .timer-ring { transition: stroke-dashoffset 1s linear; }
        .wave-bar { border-radius: 2px; animation: waveDance 1.2s ease-in-out infinite; }
        @keyframes waveDance { 0%,100% { transform: scaleY(0.3); } 50% { transform: scaleY(1); } }
        ...
        input[type=number] { -moz-appearance: textfield; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); color: inherit; border-radius: 10px; }
      ...
      {darkMode && (<><div className="orb orb-1"/><div className="orb orb-2"/><div className="orb orb-3"/></>)}
      {darkMode && (
        <div className="particles">
          {[...Array(20)].map((_,i) => (
            <div key={i} className="particle" style={{
              left:`${Math.random()*100}%`, animationDuration:`${8+Math.random()*15}s`,
              animationDelay:`${Math.random()*10}s`, width:`${1+Math.random()*2}px`,
              height:`${1+Math.random()*2}px`, opacity:0.3+Math.random()*0.4,
            }}/>
          ))}
        </div>
      )}
      ...
      {bgEnabled && (
        <div className="absolute inset-0 z-0">
          <img src={backgrounds[bgIndex].url} alt={backgrounds[bgIndex].name}
      ...
      <div className="fixed top-3 left-3 z-50 flex gap-2 items-start" ref={lightPanelRef}>
        {darkMode ? (
          <div className="relative">
            <button onClick={() => setShowLightPanel(p => !p)}
            ...
            {showLightPanel && (
              <div className="absolute top-12 left-0 z-50 w-72 rounded-2xl overflow-hidden shadow-2xl border border-white/10"
                ...
                <div className="px-4 pt-4 pb-2">
                  ...
                  <button
                    onClick={() => { setLightBgChoice('none'); setBgEnabled(false); setDarkMode(false); setShowLightPanel(false); }}
                    className={`w-full mb-3 rounded-xl border-2 transition-all duration-200 flex items-center gap-3 p-2 text-left ${lightBgChoice==='none'&&!darkMode ? 'border-emerald-400 ring-2 ring-emerald-400/30' : 'border-white/10 hover:border-emerald-400/50'}`}
                  >
                    ...
                    {lightBgChoice==='none'&&!darkMode&&<svg className="ml-auto w-4 h-4 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>}
                  </button>
                  <div className="flex flex-col gap-2 pb-3 max-h-64 overflow-y-auto">
                    {backgrounds.map((bg,idx) => (
                      <button key={idx}
                        onClick={() => { setLightBgChoice(idx); setBgIndex(idx); setBgEnabled(true); setDarkMode(false); setShowLightPanel(false); }}
                        className={`w-full rounded-xl border-2 transition-all duration-200 flex items-center gap-3 p-2 text-left ${lightBgChoice===idx&&!darkMode ? 'border-emerald-400 ring-2 ring-emerald-400/30' : 'border-white/10 hover:border-emerald-400/50'}`}
                      >
                        ...
                        {lightBgChoice===idx&&!darkMode&&<svg className="ml-auto w-4 h-4 text-emerald-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <button onClick={() => { setDarkMode(true); setBgEnabled(false); setLightBgChoice('none'); }}
          ...
          >
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 0112.21 3a7 7 0 100 14A9 9 0 0021 12.79z" fill="currentColor"/></svg>
            <span className="hidden md:inline text-xs">Dark</span>
          </button>
        )}
        {!darkMode && (
          <button onClick={() => setBgEnabled(b=>!b)} className="p-2.5 rounded-xl font-medium text-xs transition-all duration-300 bg-white/80 text-slate-600 border border-emerald-200 hover:border-emerald-400">
      ...
      {darkMode && bgEnabled && (
        <div className="fixed top-3 right-3 z-50 flex gap-1.5 max-w-[calc(100vw-120px)] overflow-x-auto pb-1">
          {backgrounds.map((bg,idx) => (
            <button key={idx} onClick={() => setBgIndex(idx)} title={bg.name}
              className={`flex-shrink-0 w-10 h-10 md:w-11 md:h-11 rounded-xl overflow-hidden border-2 transition-all duration-300 ${
                bgIndex===idx ? 'border-cyan-400 scale-110 ring-2 ring-cyan-400/30' : 'border-white/10 opacity-60 hover:opacity-90'
              }`}
            >
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-start pt-20 pb-24 px-4">
        ...
        <div className={`w-full max-w-2xl mb-8 rounded-2xl p-5 text-center text-sm leading-relaxed ${darkMode ? 'glass-dark border border-cyan-500/15 text-slate-300' : 'light-card text-slate-600'}`}>
          <span className={`text-xs tracking-[0.3em] uppercase block mb-2 ${darkMode ? 'text-cyan-400' : 'text-emerald-600'}`}>✦ Meditation Guidance ✦</span>
        ...
        <div className={`w-full max-w-2xl rounded-3xl p-6 md:p-8 mb-6 ${darkMode ? 'glass-dark' : 'light-card'}`}>
          <div className="flex items-center gap-3 mb-7">
            ...
            <h2 className={`font-display text-base tracking-[0.2em] uppercase ${darkMode ? 'text-cyan-300' : 'text-emerald-700'}`}>Sound Player</h2>
            ...
            {soundCards.map(({ key, label, color, glow, icon, activeRing }) => {
              ...
              return (
                <div key={key} className="flex flex-col items-center gap-3">
                  <button onClick={() => toggleSound(key)}
                    className={`sound-btn w-20 h-20 rounded-2xl flex flex-col items-center justify-center gap-1 text-white font-semibold bg-gradient-to-br ${color}
                  ...
                  >
                    ...
                    {isActive && (
                      <div className="flex items-end gap-0.5 h-4">
                        {[...Array(5)].map((_,i) => (
                          <div key={i} className="wave-bar w-1 bg-white/80" style={{ height:`${8+Math.random()*8}px`, animationDelay:`${i*0.15}s` }}/>
                        ))}
                      </div>
                    )}
                  </button>
                  <span className={`text-xs text-center font-medium tracking-wide ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{label}</span>
        ...
        <div className={`w-full max-w-2xl rounded-3xl p-6 md:p-8 mb-6 ${darkMode ? 'glass-dark' : 'light-card'}`}>
          ...
          <div className="flex flex-col gap-3">
            {freqCards.map(({ key, hz, label, color, border, glow, badge }) => {
              ...
              return (
                <button key={key} onClick={() => toggleFrequency(key)}
                  className={`freq-btn relative w-full rounded-2xl p-4 flex items-center justify-between text-left transition-all duration-300 border ${
                    isActive ? `bg-gradient-to-r ${color} bg-opacity-20 ${border} shadow-lg ${glow} active`
                      : darkMode ? `glass-dark ${border} hover:border-white/20` : `light-card ${border}`
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className={`text-xs mt-0.5 ${isActive?'text-white/70':darkMode?'text-slate-500':'text-slate-400'}`}>{label}</div>
                    </div>
                  </div>
        ...
        <div className="w-full max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
          <div className={`rounded-3xl p-6 flex flex-col items-center gap-5 ${darkMode?'glass-dark':'light-card'}`}>
            <h2 className={`font-display text-sm tracking-[0.2em] uppercase ${darkMode?'text-cyan-300':'text-emerald-700'}`}>Breathing Guide</h2>
            <div className="relative flex items-center justify-center w-32 h-32">
              ...
              <div className={`breath-orb w-20 h-20 bg-gradient-to-br ${
                breathPhase==='inhale'?'from-cyan-400 to-blue-600':breathPhase==='hold'?'from-violet-400 to-purple-600':'from-emerald-400 to-teal-600'
              } ${breathPhase}`}/>
            ...
            <p className={`text-xs text-center ${darkMode?'text-slate-500':'text-slate-400'}`}>4s inhale · 4s hold · 4s exhale</p>
            <button onClick={() => setBreathActive(a=>!a)}
              className={`px-6 py-2.5 rounded-xl text-sm font-semibold tracking-wide transition-all duration-300 ${
                breathActive
                  ? darkMode?'bg-gradient-to-r from-cyan-500 to-violet-500 text-white shadow-lg shadow-cyan-500/20':'bg-emerald-600 text-white'
                  : darkMode?'glass-dark border border-white/10 text-slate-300 hover:border-cyan-500/30 hover:text-cyan-300':'border border-emerald-300 text-emerald-700 hover:bg-emerald-50'
              }`}
            >{breathActive ? '⏸ Pause' : '▶ Begin'}</button>
          </div>

          <div className={`rounded-3xl p-6 flex flex-col items-center gap-5 ${darkMode?'glass-dark':'light-card'}`}>
            <h2 className={`font-display text-sm tracking-[0.2em] uppercase ${darkMode?'text-violet-300':'text-emerald-700'}`}>Session Timer</h2>
            <div className="relative flex items-center justify-center w-32 h-32">
              <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 120 120">
                <circle cx="60" cy="60" r="54" fill="none" stroke={darkMode?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.06)'} strokeWidth="4"/>
                <circle cx="60" cy="60" r="54" fill="none" stroke={darkMode?'url(#timerGrad)':'#10b981'} strokeWidth="4" strokeLinecap="round"
                  strokeDasharray={circumference} strokeDashoffset={circumference*(1-timerProgress)} className="timer-ring"/>
                <defs>
              ...
              <div className={`font-display text-2xl font-bold ${darkMode?'text-white':'text-slate-800'}`}>{formatTime(timerSeconds)}</div>
            </div>
            <div className="flex items-center gap-2 w-full justify-center">
              <input type="number" min="1" max="60" value={timerMinutes}
                onChange={e=>setTimerMinutes(Number(e.target.value))} disabled={timerRunning}
                className={`w-16 text-center text-sm py-2 rounded-xl focus:outline-none focus:ring-1 focus:ring-cyan-400 ${darkMode?'bg-white/5 border border-white/10 text-white':'bg-white border border-slate-200 text-slate-800'}`}
              />
              <span className={`text-xs ${darkMode?'text-slate-500':'text-slate-400'}`}>min</span>
              {!timerRunning ? (
                <button onClick={startTimer}
                  className={`px-5 py-2 rounded-xl text-sm font-semibold tracking-wide transition-all duration-300 ${darkMode?'bg-gradient-to-r from-cyan-500 to-violet-500 text-white hover:shadow-lg hover:shadow-cyan-500/25 hover:scale-105':'bg-emerald-600 text-white hover:bg-emerald-700'}`}>
                  Start
                </button>
              ) : (
                <button onClick={stopTimer}
                  className={`px-5 py-2 rounded-xl text-sm font-semibold tracking-wide transition-all duration-300 ${darkMode?'bg-[#517c61] border border-yellow-500/40 text-black':'bg-rose-100 border border-rose-300 text-rose-600'}`}>
                  Stop
                </button>
              )}
            </div>
      ...
      <footer className={`w-full text-center py-2.5 text-xs fixed bottom-0 left-0 z-50 ${darkMode?'bg-black/60 backdrop-blur text-slate-600 border-t border-white/5':'bg-white/80 backdrop-blur text-slate-400 border-t border-slate-100'}`}>
